...
export default function MonthlySales() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/reports/monthly-sales", {
          headers: { auth: token },
          params: { companyId },
        });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="text-2xl font-bold mb-4">Monthly Sales</h1>
          ...
          <tbody>
            {monthly.map((m, i) => (
              <tr key={i} className="border-t">
                ...
                <td className="p-3">₹{m.total.toFixed(2)}</td>
              </tr>
            ))}
            {monthly.length === 0 && (
